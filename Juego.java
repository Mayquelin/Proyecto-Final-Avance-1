...
public class Juego {
   ...
   // Reparte 8 cartas a la mano desde mazo 
    ...
    /**    valida una tripleta de cartas seleccionada por el jugador
      @param c1 primera carta
      @param c2 segunda carta
      @param c3 tercera carta
      @return número de cartas a robar según reglas:
       4 = mismo palo, 3 = mismo color, 2 = distinto color, 0 = no válido
     */
    ...
    /**
     * Método recursivo que revisa todas las permutaciones de la tripleta.
     * @param cartas arreglo de 3 cartas
     * @param index índice actual para permutar
     * @param maxCartas máximo número de cartas a robar encontrado
     * @return el máximo número de cartas a robar según las reglas
     */
    private int validarPermutaciones(Carta[] cartas, int index, int maxCartas) {
        if (index == cartas.length) {
            // Evaluar la permutación actual
        ...
        for (int i = index; i < cartas.length; i++) {
            // Intercambiar para crear permutación
